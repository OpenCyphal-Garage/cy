...
The eviction counter itself is used as a Lamport clock and is not expected to overflow, as it is wide enough; for reference, it will take 136 years of continuous churn at 1 Hz to overflow a 32-bit eviction counter (eviction counters are not incremented continuously but only when the network configuration is changed in a way that triggers collisions, so the limit is unreachable in any practical scenario). Formal models assume that eviction counters never exceed $2^{32}-1$.
