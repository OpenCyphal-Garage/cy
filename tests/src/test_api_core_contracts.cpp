...
struct diag_event_t
{
    ...
    const cy_diag_t*                         listener{ nullptr };
...
diag_collector_t* collector_from_diag(cy_diag_t* const diag)
{
    TEST_ASSERT_NOT_NULL(diag);
    auto* const out = static_cast<diag_collector_t*>(diag->user_context.ptr[0]);
    TEST_ASSERT_NOT_NULL(out);
    return out;
}

std::size_t* counter_from_diag(cy_diag_t* const diag)
{
    TEST_ASSERT_NOT_NULL(diag);
    auto* const out = static_cast<std::size_t*>(diag->user_context.ptr[1]);
    TEST_ASSERT_NOT_NULL(out);
    return out;
}
...
void diag_push(cy_diag_t* const        listener,
               ...
               const cy_str_t          name)
{
    diag_collector_t* const collector = collector_from_diag(listener);
    if (collector->count >= collector->events.size()) {
        return;
    }
    diag_event_t& out = collector->events.at(collector->count++);
    out               = diag_event_t{};
    out.kind          = kind;
...
extern "C" void on_diag_one_topic_reallocated(cy_diag_t* const    diag,
                                              cy_topic_t* const   topic,
                                              const std::uint32_t subject_id,
                                              const std::uint32_t evictions)
{
    (*counter_from_diag(diag))++;
    diag_push(diag,
...
extern "C" void on_diag_one_topic_created(cy_diag_t* const diag, cy_topic_t* const topic)
{
    (*counter_from_diag(diag))++;
    diag_push(diag, diag_event_kind_t::created, topic, cy_topic_hash(topic), 0U, 0U, CY_OK, 0U, cy_str_t{ 0, nullptr });
}

extern "C" void on_diag_one_topic_destroyed(cy_diag_t* const diag, cy_topic_t* const topic)
{
    (*counter_from_diag(diag))++;
    diag_push(
      diag, diag_event_kind_t::destroyed, topic, cy_topic_hash(topic), 0U, 0U, CY_OK, 0U, cy_str_t{ 0, nullptr });
}

extern "C" void on_diag_one_gossip_processed(cy_diag_t* const    diag,
                                             cy_topic_t* const   topic,
                                             const cy_str_t      name,
                                             const std::uint64_t hash)
{
    (*counter_from_diag(diag))++;
    diag_push(diag, diag_event_kind_t::gossip_processed, topic, hash, 0U, 0U, CY_OK, 0U, name);
}

extern "C" void on_diag_one_async_error(cy_diag_t* const    diag,
                                        cy_topic_t* const   topic,
                                        const cy_err_t      error,
                                        const std::uint16_t line_number)
{
...
extern "C" void on_diag_two_topic_reallocated(cy_diag_t* const    diag,
                                              cy_topic_t* const   topic,
                                              const std::uint32_t subject_id,
                                              const std::uint32_t evictions)
{
...
extern "C" void on_diag_two_topic_created(cy_diag_t* const diag, cy_topic_t* const topic)
{
    (void)topic;
    (*counter_from_diag(diag))++;
}

extern "C" void on_diag_two_topic_destroyed(cy_diag_t* const diag, cy_topic_t* const topic)
{
    (void)topic;
    (*counter_from_diag(diag))++;
}

extern "C" void on_diag_two_gossip_processed(cy_diag_t* const    diag,
                                             cy_topic_t* const   topic,
                                             const cy_str_t      name,
                                             const std::uint64_t hash)
{
...
extern "C" void on_diag_two_async_error(cy_diag_t* const    diag,
                                        cy_topic_t* const   topic,
                                        const cy_err_t      error,
                                        const std::uint16_t line_number)
{
...
void test_api_core_diag_listener_contracts()
{
    ...
    cy_diag_t listener_one{ .next         = nullptr,
                            ...
                            .vtable       = &diag_listener_one_vtable };
    listener_one.user_context.ptr[0] = &collector;
    listener_one.user_context.ptr[1] = &collector.listener_one_calls;
    cy_diag_t listener_two{ .next         = nullptr,
                            .user_context = CY_USER_CONTEXT_EMPTY,
                            .vtable       = &diag_listener_two_vtable };
    listener_two.user_context.ptr[0] = &collector;
    listener_two.user_context.ptr[1] = &collector.listener_two_calls;
    ...
    TEST_ASSERT_TRUE(listener_one.user_context.ptr[0] == &collector);
    TEST_ASSERT_TRUE(listener_one.user_context.ptr[1] == &collector.listener_one_calls);
    TEST_ASSERT_TRUE(listener_two.user_context.ptr[0] == &collector);
    TEST_ASSERT_TRUE(listener_two.user_context.ptr[1] == &collector.listener_two_calls);
    ...
    TEST_ASSERT_TRUE(collector.events.at(0U).listener == &listener_one);
    TEST_ASSERT_TRUE(collector.events.at(1U).listener == &listener_one);
    ...
    TEST_ASSERT_TRUE(collector.events.at(2U).listener == &listener_one);
    ...
    TEST_ASSERT_TRUE(collector.events.at(3U).listener == &listener_one);
    ...
    TEST_ASSERT_TRUE(collector.events.at(4U).listener == &listener_one);
