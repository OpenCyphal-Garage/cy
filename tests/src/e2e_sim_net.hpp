...
{
    std::array<sim_node_t, sim_node_count> nodes{};
    ...
    const fault_plan_t* faults{ nullptr };
};
...
void sim_net_faults_set(sim_net_t& self, const fault_plan_t* faults);
...
cy_us_t       sim_net_node_now(const sim_net_t& self, std::size_t node_index);
